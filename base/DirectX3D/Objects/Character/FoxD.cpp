...
void FoxD::Control()
{
    ...
    if (KEY_DOWN(VK_SPACE)) // 스페이스바를 누르면
    {
        ...
        if (terrain->ComputePicking(destination)) SetPath(); // 목적지가 제대로 찍혀야 경로설정이 가능하게
    }
...
void FoxD::Move()
{
    if (path.empty()) //경로 벡터가 비어 있는 경우
    {
        ...
        firstLocation = GlobalPos(); //시작 위치가 존재할 경우, 멈출 때마다 동기화
...
void FoxD::SetPath()
{
    int start = dijkstra->FindCloseNode(firstLocation); // 여우에서 가장 가까운 노드를 출발점으로
    //int end = dijkstra->FindCloseNode(terrain->Picking()); // 마우스 커서 근처를 종점으로
    int end = dijkstra->FindCloseNode(destination);
